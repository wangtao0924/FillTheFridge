...
namespace MPStudio
{
    ...
    /// 变换组件扩展
    /// </summary>
    public static class MPExpandTransform
    {
        /// <summary>
        /// 销毁所有孩子
        /// </summary>
        ...
        public static void DestroyAllChild(this Transform Root, bool InOneFrame = false)
        {
            if (Root.childCount != 0)
            {
                ...
                {
                    for (int i = Root.childCount - 1; i >= 0; i--)
                        MPRes.ReleasePrefab(Root.GetChild(i).gameObject);
                }
            }
        }
        ...
        public static T FindChildComponent<T>(this Transform target, string ChildName) where T : Component
        {
            ...
            {
                MPLOG.E($"in {target.name} can not find child {ChildName}");
                return (T)null;
            }
        ...
        /// <returns></returns>
        public static Transform DeepFindChild(this Transform target, string targetName)
        {
