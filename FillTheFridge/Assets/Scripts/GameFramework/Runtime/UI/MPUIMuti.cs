...
namespace MPStudio
{
    ...
    /// </summary>
    public class MPUIMuti<T> : MPUIBase where T : MPUIMuti<T>
    {
        /// <summary>
        /// UI实例
        /// </summary>
        ...
        public static T CreateUI(Transform parent = null)
        {
            ...
            _uinsts.Add(ui);
            var uiname = MPFile.GetFileName(bindInfo.Prefab);
        ...
        /// 干掉所有UI
        /// </summary>
        /// <param name="immediate">是否在一帧内干掉所有</param>
        public static void DestroyAllUI(bool immediate = false)
        {
            // 需要在一帧内干掉所有
            if (immediate)
            {
        ...
        public static async Task DestroyUIWithAnim(CUIAnimOut OutAnim, T target)
        {
            ...
            {
                // 播完动画再销毁
                await MPUIAnimtaion.PlayOutAnim(target.rectTransform, OutAnim);
                DestroyUI(target);
            }
        }
