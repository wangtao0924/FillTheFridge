            ...
            for (int i = 0; i < catalogOps.Count; i++)
            {
                ...
                foreach (IList<IResourceLocation> locationList in locator.Locations.Values)
                {
                    foreach (IResourceLocation location in locationList)
                    {
                        ...
                        {
                            IDataConverter dataProc = null;
                            AssetBundleProvider assetBundleProvider = m_Addressables.ResourceManager.GetResourceProvider(null, location) as AssetBundleProvider;
                            if (assetBundleProvider != null)
                            {
                                dataProc = assetBundleProvider.DataStreamProcessor;
                            }

                            GetLoadInfo(location, m_Addressables.ResourceManager, dataProc, out LoadType loadType, out string path);
